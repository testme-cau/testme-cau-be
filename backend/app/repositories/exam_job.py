"""
Repository for exam generation jobs stored under each subject
Path: users/{user_id}/subjects/{subject_id}/exam_jobs/{job_id}
"""
...
class ExamJobRepository:
    """Firestore repository for exam generation jobs."""
    ...
    def create_job(self, user_id: str, subject_id: str, job_data: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            raise HTTPException(
                ...
                detail=f"Failed to create exam job: {str(e)}"
            )
    ...
    def list_jobs(self, user_id: str, subject_id: str) -> List[Dict[str, Any]]:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to list exam jobs: {str(e)}"
            )
    ...
    def get_job(self, user_id: str, subject_id: str, job_id: str) -> Dict[str, Any]:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to get exam job: {str(e)}"
            )
    ...
    def update_job(self, user_id: str, subject_id: str, job_id: str, data: Dict[str, Any]) -> None:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to update exam job: {str(e)}"
            )
    ...
    def delete_job(self, user_id: str, subject_id: str, job_id: str) -> None:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to delete exam job: {str(e)}"
            )
